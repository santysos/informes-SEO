#!/usr/bin/env python3
"""Publica el lote de posts de OKCars (septiembre 2026) vía WP REST API.

OJO: okcars.ec tiene un WAF agresivo. Los delays son deliberadamente altos
(12 s entre llamadas, 30 s entre publicaciones) y ante un bloqueo se espera 120 s.
...
Credenciales en ../../.env: OKCARS_WP_BASE, OKCARS_WP_USER, OKCARS_WP_APP_PASS.
"""
...
DELAY = 12         # entre llamadas sueltas (WAF de okcars.ec)
DELAY_POST = 30    # entre publicaciones completas
BACKOFF = 120

CATEGORIAS = {
    42: "Guías de compra",
    43: "Modelos y comparativas",
    44: "Financiamiento",
    45: "Trámites",
    46: "Seminuevos en Ibarra",
}
...
class Api:
    def __init__(self, env):
        self.base = env["OKCARS_WP_BASE"].rstrip("/")
        self.auth = (env["OKCARS_WP_USER"], env["OKCARS_WP_APP_PASS"])
...
def validar(spec):
    ...
    refs = sum(1 for r in IBARRA_REFS if r in low)
    if refs < 2:
    ...
    internos = len(re.findall(r'href="https://okcars\.ec/', c))
    if internos < 2:
